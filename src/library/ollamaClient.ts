...
// hardcoded literal. Muse Glimmer is now the one model serving everything
// (chat, quiz/flashcards, advising, OCR - see resolveModelFromKey), so
// reloading it from cold costs more than it did when the load was spread
// across several models, and there's no second large model's VRAM footprint
// to weigh against keeping it around. Defaults to "2h" if the env var is
// unset. This explicit per-request value overrides the daemon-level
// OLLAMA_KEEP_ALIVE default (1h on both boxes as of 2026-08-15).
