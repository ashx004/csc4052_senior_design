...
function DocumentPreviewModal({
  ...
  const isOcrDocument = doc.resourceKind === "ocr_document";
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, "users", userId, "enrollment", doc.classId, "resources", doc.id, "pages"))
      .then((snapshot) => setSourceImages(snapshot.docs.sort((a, b) => Number(a.data().order ?? 0) - Number(b.data().order ?? 0)).map((page) => String(page.data().url ?? "")).filter(Boolean)))
      .catch(() => setSourceImages([]));
  }, [userId, doc.classId, doc.id, isOcrDocument]);
      ...
      >
        <div className="flex items-center justify-between border-b border-border-light px-4 py-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            {isOcrDocument && <button type="button" onClick={() => setShowSourceImages((show) => !show)} className="rounded-md border border-border-light px-3 py-1.5 text-xs font-medium text-text-main hover:bg-bg-main">{showSourceImages ? "View transcript" : "View source images"}</button>}
            <a
              href={doc.url}
              download
        ...
        <div className="relative flex-1 overflow-auto bg-bg-container">
          {isOcrDocument && showSourceImages ? (
            <div className="space-y-4 p-4">{sourceImages.length ? sourceImages.map((url, index) => <img key={url} src={url} alt={`Source scan ${index + 1}`} className="mx-auto max-w-full rounded-lg border border-border-light shadow-sm" />) : <p className="p-6 text-center text-sm text-text-muted">Source images are not available yet.</p>}</div>
          ) : isPdf ? (
...
export default function Notes() {
  ...
  const [classFilter, setClassFilter] = useState(requestedCourseId ?? "all");
      ...
      onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.forEach((resourceDoc) => {
            ...
            if (resource.category !== "notes" || (resource.resourceKind !== "ocr_document" && resource.ocrScanned !== true)) return;
            ...
            classNotes.push({
              ...
              resourceKind: resource.resourceKind === "ocr_document" ? "ocr_document" : undefined,
            });
          });
  ...
  return (
    <section className="min-h-screen bg-bg-main px-8 py-8 text-text-main">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <nav className="mb-6 flex gap-1 border-b border-border-light" aria-label="Notes sections">
          <Link href={requestedCourseId ? `/courses/${requestedCourseId}/notes` : "/notes"} className="border-b-2 border-transparent px-3 py-2 text-sm text-text-muted hover:text-text-main">Notes</Link>
          <Link href={requestedCourseId ? `/notes/ocr?courseId=${requestedCourseId}` : "/notes/ocr"} className="border-b-2 border-primary px-3 py-2 text-sm font-medium text-primary">OCR</Link>
        </nav>
