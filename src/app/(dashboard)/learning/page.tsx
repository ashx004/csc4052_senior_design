  ...
  return (
    <div className="min-h-screen bg-bg-main px-6 py-10 sm:px-10">
      <div className="mx-auto max-w-5xl">
        <h1 className="mb-8 text-2xl md:text-3xl font-bold text-text-main tracking-tight">
          Select a class to start learning
        </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {classes.map((cls) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-text-muted">{cls.classCode}</p>
                {cls.term && <p className="mt-0.5 text-xs text-text-muted">{cls.term}</p>}
              </button>
            ))}
          </div>
        )}
