...
export default function ShareNoteModal({ type, id, onClose }: { type: "note" | "notebook"; id: string; onClose: () => void }) {
  ...
  async function create() {
    ...
    try {
      const response = await fetch("/api/note-shares", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, id, password }) });
